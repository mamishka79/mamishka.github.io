class Game {
    constructor(canvas, context, canvas2, context2){
        ...
        window.addEventListener('keyup', e => {
            if (e.key === '[' || e.key === 'х') this.toggleFullScreen();
            else if (e.key === ']' || e.key === 'ъ') this.debug = !this.debug;
        });
    ...
    initPlayer3(){
        ...
        if (this.gameUi.player3controls.value === 'ikjl'){
            this.player3 = new Keyboard3(this, this.columns - 1, this.rows-1, -1, 0, 'yellow', name, image);
        } else {
    ...
    initPlayer4(){
        ...
        if (this.gameUi.player4controls.value === '8546'){
            this.player4 = new Keyboard4(this, 0, this.rows-1, 0, -1, 'darkblue', name, image);
        } else {
